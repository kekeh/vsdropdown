...
angular.module("vsscrollbar", ["template-vsdropdown-0.0.4.html"]).constant("vsscrollbarConfig", {
    ITEMS_IN_PAGE: 6,
    SCROLLBAR_HEIGHT: 0,
...
}).service("vsdropdownService", ["$http", "$templateCache", function (a, b) {
    this.getTemplate = function (c) {
        var d = a.get(c, {cache: b}).success(function (a) {
            return a.data
        });
        return d
    }
    ...
    return {
        ...
        link: function (c, d, e) {
            ...
            function h() {
                if (c.options.input.isObject) {
                    var a = {};
                    b.filter(c, i(a))
                } else b.filter(c, c.filterText)
            }

            function i(a) {
                for (var b = c.visiblePropName.split(c.config.DOT_SEPARATOR), d = b.pop(), e = 0, f = a; e < b.length; e++)f = f[b[e]] = {};
                return f[d] = c.filterText, a
            }
            ...
            function l(a, b) {
                var d = a.split(c.config.DOT_SEPARATOR), e = angular.copy(b);
            ...
            c.selector = function () {
                ...
                b.stopPropagation(), c.options.selection.maximum > 1 ? (c.selectedItems.length === c.options.selection.maximum && c.removeItem(c.selectedItems.length - 1, b), c.selectedItems.push(a)) : c.selectedItems[0] = a, j(a, c.config.OPERATION_ADD)
            }, c.removeItem = function (a, b) {
                b.stopPropagation();
                var d = c.selectedItems[a];
                (a === c.selectedItems.length - 1 || 2 === c.selectedItems.length) && c.closeOverlay(), c.selectedItems.splice(a, 1), j(d, c.config.OPERATION_DEL)
            }, c.isItemSelected = function (a) {
                return -1 !== c.selectedItems.indexOf(a)
            }, c.getPropertyValue = function (a, b) {
                return null === a ? b : -1 === a.indexOf(c.config.DOT_SEPARATOR) ? b[a] : l(a, b)
                ...
                n || ((13 === a.which || 38 === a.which || 40 === a.which || 27 === a.which) && a.preventDefault(), 13 === a.which && c.focusIdx > -1 ? c.itemClicked(c.focusIdx, a) : 38 === a.which ? 0 === c.focusIdx ? (b.setIndex(c, c.topIndex - c.options.visibleItemCount), c.focusIdx = c.filteredItemCount < c.options.visibleItemCount ? c.filteredItemCount - 1 : c.options.visibleItemCount - 1) : c.focusIdx-- : 40 === a.which ? k() ? (b.setIndex(c, c.topIndex + c.options.visibleItemCount), c.focusIdx = 0) : c.focusIdx++ : 27 === a.which && (c.showSelector = !1))
            }, c.clearFilter = function () {
                c.filterText = "", c.listFocusEvent()
            };
            var o = c.$watch("filterText", f), p = c.$watch("options.items.length", g);
...
}]).directive("tooltipWindow", ["$compile", "$timeout", "vsdropdownService", function (a, b, c) {
    return {
        restrict: "A", scope: !1, link: function (d, e, f) {
            function g() {
                e[0].scrollWidth > e[0].offsetWidth && (m = b(function () {
                    c.getTemplate("dropdowntooltip.html").then(function (b) {
                        l = angular.element(b.data), e.append(a(l)(d))
                    })
                }, d.config.TOOLTIP_OPEN_DELAY))
            }

            function h() {
                i(), angular.equals(l, null) || (l.remove(), l = null)
            }
            ...
            function j(a, b) {
                angular.equals(a, b) || h()
            }

            function k() {
                d.options.showTooltip && (e.on("mouseenter", g), e.on("mouseleave", h), n = d.$watch("topIndex", j))
            }

            var l = null, m = null, n = null;
            d.closeTooltip = function (a) {
                a.stopPropagation(), h()
            }, d.$on("$destroy", function () {
                e.off("mouseenter", g), e.off("mouseleave", h), angular.equals(n, null) || n()
            }), k()
        }
    }
}]).directive("popoverWindow", ["$compile", "vsdropdownService", function (a, b) {
    return {
        restrict: "A", scope: !1, link: function (c, d, e) {
            function f(a, b) {
                angular.equals(a, b) || c.closeProperties()
            }

            function g() {
                h = c.$watch("topIndex", f)
            }

            c.popover = null;
            var h = null;
            c.showProperties = function (e) {
                e.stopPropagation(), angular.equals(c.popover, null) ? b.getTemplate("dropdownpopover.html").then(function (b) {
                    c.popover = angular.element(b.data), d.append(a(c.popover)(c))
                }) : c.closeProperties()
            }, c.closeProperties = function () {
                angular.equals(c.popover, null) || (c.popover.remove(), c.popover = null)
            }, c.$on("$destroy", function () {
                angular.equals(h, null) || h()
            }), g()
        }
    }
